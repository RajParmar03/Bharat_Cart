    ...
    <>
      <Navbar />
      {
        loadingManager.isLoading ?
          ...
          :
          <>
            <VStack m={"130px auto auto auto"} alignItems={"end"} w={"90%"}>
              <Button w={"10%"} onClick={() => handleLogout()}>Log-out</Button>
              <Button w={"10%"} onClick={() => navigate("/orderlist")}>Order-List</Button>
            </VStack>
            <Box m={"auto auto 30px auto"} p={"30px"} w={"40%"} boxShadow="rgba(0, 0, 0, 0.24) 0px 3px 8px">
              ...
              <TableContainer>
                    <Table variant='simple'>
                      <Tbody>
                        <Tr>
                          <Td color={"orange"} fontSize={"20px"}>Name</Td>
                          <Td fontSize={"20px"}>{user.name}</Td>
                          
                        </Tr>
                        <Tr>
                          <Td color={"orange.400"} fontSize={"20px"}>User Name</Td>
                          <Td fontSize={"20px"}>{user.username}</Td>
                          
                        </Tr>
                        <Tr>
                          <Td color={"orange.400"} fontSize={"20px"}>Email</Td>
                          <Td fontSize={"20px"}>{user.email}</Td>
                        </Tr>
                        <Tr>
                          <Td color={"orange.400"} fontSize={"20px"}>Mobile Number</Td>
                          <Td fontSize={"20px"}>{user.phone}</Td>
                        </Tr>
                        <Tr>
                          <Td color={"orange.400"} fontSize={"20px"}>Role</Td>
                          <Td fontSize={"20px"}>{user.role}</Td>
                        </Tr>
                      </Tbody>
                    </Table>
                  </TableContainer>
